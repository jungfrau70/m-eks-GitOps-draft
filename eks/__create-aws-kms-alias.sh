...
# Create
aws kms create-alias --alias-name alias/cloud9 --target-key-id $(aws kms create-key --query KeyMetadata.Arn --output text)
...
# Export ENV
export MASTER_ARN=$(aws kms describe-key --key-id alias/cloud9 --query KeyMetadata.Arn --output text)
